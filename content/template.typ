...
@seq-pseudo-code gives the whole sequential program. Setup runs once and is described in @sec-setup-seq. The time loop is four operations repeated 50 000 times and is described in @sec-timeloop. Everything else in this chapter expands one part of this listing.
...
#figure(
  ...
  SETUP (once)
      ...
      run both kernels once, then zero the fields        # fault in pages,
...
=== The  Python and C split

We divided the program into two splits, Python performs setup, orchestration and I/O, while C performs the two kernels that the time loop invokes 50000 times. The MPI calls are in Python as `mpi4py` transfers NumPy buffers directly, so the halo exchange of costs no more there than it would in C, and keeping it out of the kernel leaves the kernel free of communication entirely. The primary reason for this split is to improve comparability between the sequential and parallel measurements.

=== Division of responsibilities

Setup reads the SEG-Y models, constructs the material arrays, computes the damping ramp and initialises the output file. It executes once, consists largely of array manipulation, and is expressed far more concisely in NumPy than it would be in C. The time loop performs a single operation repeatedly and is sensitive to every cycle, which justifies writing it by hand.

Communication sits on the Python side with the rest of the orchestration. mpi4py transfers NumPy buffers directly to the MPI library, so the halo exchange described in @halo costs no more there than an equivalent call from C would. The benefit is that the kernel contains no communication at all. It receives a tile and a set of loop bounds, and has no knowledge of ranks, neighbours or topology, which is what allows the same kernel source to serve both versions. 

=== Calling the kernel <seq-calling-kernel>


The kernels are invoked through `ctypes`. NumPy's `ndpointer` declares each argument as a two-dimensional C-contiguous `float32` array, so arrays are passed as raw pointers without copying or conversion.
...
#figure(
  ...
  ```,
  caption: [
    Binding the C kernel. The call overhead is a few microseconds against a
    kernel that executes for milliseconds.
  ],
) <lst-ctypes>

The path supplied to `CDLL` is the only place the two versions differ on the
Python side, since each loads its own build of the kernel.

=== Setup <sec-setup-seq>

We reading the model using `segyio`, which returns the traces which are stacked and transposed into $(n_z, n_x)$ order.
...
#figure(
  ...
  ```,
  caption: [
    Reading one SEG-Y file. Trace geometry is ignored, since only the sample
    values are required.
  ],
) <lst-segy>

The model is then padded by $n_b = 240$ cells on each side and then the parameters are derived.
...
Next, the damping array is constructed.
...
#figure(
  ...
  r = ramp(pad_left)
  for i in range(pad_left):                      # left edge; other three alike
  ...
  ```,
  caption: [
    Construction of the sponge layer. Two sides are omitted, as they mirror the
    left edge.
  ],
) <lst-damping>


=== The time loop <sec-timeloop>

The source is a Ricker wavelet evaluated at the current time and added to both diagonal stress components at a single grid point. In the parallel version a single rank owns that point; in the sequential case it is always the only rank.
...
#figure(
  ...
  ```,
  caption: [
    The Ricker source . Adding to $sigma_(x x)$ and $sigma_(z z)$
    but not to $sigma_(x z)$ renders the source isotropic.
  ],
) <lst-source>

The complete time loop consists of four statements.
...
@lst-stresskernel gives the stress kernel. The kernel comprises a row loop over $i$ and a column loop over $j$, with the row offsets comes out of the inner loop.
...
#figure(
  ...
  ```,
  caption: [
    The stress update kernel.).
  ],
) <lst-stresskernel>

The damping in applied in the same pass as the update rather than in separate sweep which saves reading and writing three arrays at every step. 
...
=== Domain Decomposition and Halo Exchange <halo>
